/// FROM: https://github.com/jcwillox/typebox-x/blob/main/src/tools/errors.ts
import { type StaticDecode, type TSchema, TypeBoxError } from "@sinclair/typebox";
import { type ValueError } from "@sinclair/typebox/errors";
import { TransformDecodeCheckError, Value } from "@sinclair/typebox/value";
import { type MergedNumericValueError, mergeErrors } from "@/utils/tools";
...
const formatError = (e: MergedNumericValueError) =>
  `  - ${GREEN}${e.path.slice(1)}${RESET}: ${YELLOW}${String(e.value)}${RESET}, ${CYAN}` +
