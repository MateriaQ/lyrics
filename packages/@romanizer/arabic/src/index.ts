...
const romanizeArabic = (input: string): string => {
  if (!input) return "";
...
const standardizeArabicScript = (input: string): string => {
  ...
  text = text.replace(/[ٱٰکںھۀہﮨﮩﮪﮫ]/g, (match) => fallbackCharMap[match] || match);
...
const safeNaiveVowelize = (word: string): string => {
  ...
  const isVowelOrHamza = /[اويةىءأإؤئیے\u200c\u200d-]/;
  let result = "";
...
export { romanizeArabic, romanizeArabic as default, romanizeArabic as romanize };
