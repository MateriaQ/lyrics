...
// Precompiled once at module load: patterns are constant literals.
const LIGATURE_REPLACERS = ARMENIAN_LIGATURES.map(({ ligature, expansion }) => ({
  // fallow-ignore-next-line security-sink -- pattern from hardcoded ligature data
  regex: new RegExp(ligature, "g"),
  expansion,
}));
const TEMP_SYMBOL_REPLACERS = Object.entries(TEMP_SYMBOLS_MAP.forward).map(
  // fallow-ignore-next-line security-sink -- pattern from hardcoded TEMP_SYMBOLS_MAP
  ([armSequence, tempSymbol]) => ({ regex: new RegExp(armSequence, "g"), tempSymbol }),
);
...
function normalizeArmenianWord(word: string): string {
  ...
  for (const { regex, expansion } of LIGATURE_REPLACERS) {
    result = result.replace(regex, expansion);
  }

  for (const { regex, tempSymbol } of TEMP_SYMBOL_REPLACERS) {
    result = result.replace(regex, tempSymbol);
  }
...
function transliterateArmenianWordRaw(word: string): string {
  const chars = Array.from(GRAPHEME_SEGMENTER.segment(word), (s) => s.segment);
...
export { romanizeArmenian as default, romanizeArmenian as romanize, romanizeArmenian };
