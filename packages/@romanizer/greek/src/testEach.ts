...
const mapChars = (text: string, schema: Schema) =>
  Array.from(segmenter.segment(text))
    .map(({ segment }) => schema[transliterateMap[segment]] ?? segment)
    .join("");

// Precompiled once at module load: patterns are constant literals.
// fallow-ignore-next-line security-sink -- pattern from hardcoded literal array
const GAMMA_NASALS = ["γγ", "γκ", "γξ", "γχ"].map((n) => new RegExp(n));
// fallow-ignore-next-line security-sink -- pattern from hardcoded literal array
const CAPITAL_GAMMA_NASALS = ["γγ", "γκ", "γξ", "γχ"].map((n) => new RegExp(n.toUpperCase()));
// fallow-ignore-next-line security-sink -- pattern from hardcoded literal array
const DIPTHONGS = ["αυ", "ευ", "ηυ", "ου", "υι"].map((d) => new RegExp(d));
// fallow-ignore-next-line security-sink -- pattern from hardcoded literal array
const CAPITAL_DIPTHONGS = ["αυ", "ευ", "ηυ", "ου", "υι"].map((d) => new RegExp(d.toUpperCase()));
...
const gammaNasalRules = (word: string, schema: Schema, checkCapitals = false) => {
  const [gamma, kappa, xi, chi] = GAMMA_NASALS;
  ...
  if (checkCapitals) {
    const [cGamma, cKappa, cXi, cChi] = CAPITAL_GAMMA_NASALS;
...
const diphtongRules = (word: string, schema: Schema, checkCapitals = false) => {
  const [alpha, epsilon, eta, omicron, iota] = DIPTHONGS;
  ...
  if (checkCapitals) {
    const [cAlpha, cEpsilon, cEta, cOmicron, cIota] = CAPITAL_DIPTHONGS;
