...
import { Schema, romanize } from "~/index";
...
describe("using default options", () => {
  describe("basic tests", () => {
    ...
    `("$description", (inputs: Inputs) => {
      ...
      expect(romanize(hebrew)).toBe(transliteration);
    });
  });
  ...
  describe("divine name, elohim", () => {
    ...
    `("$description", (inputs: Inputs) => {
      ...
      expect(romanize(hebrew, options)).toBe(transliteration);
    });
  });
...
describe("extending SBL schema for optional arguments", () => {
  describe("syllable separator", () => {
    ...
    `("$description", (inputs: Inputs) => {
      const { hebrew, transliteration } = inputs;
      expect(romanize(hebrew, { SYLLABLE_SEPARATOR: "-" })).toBe(transliteration);
    });
  });
  ...
  test("syllable separator when two vowles are next to each other", () => {
    ...
    expect(romanize(hebrew, { ALEF: "", SYLLABLE_SEPARATOR: ".", TSERE: "ee" })).toBe(
      transliteration,
    );
  });
