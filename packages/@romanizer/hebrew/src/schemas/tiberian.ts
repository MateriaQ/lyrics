...
const transliterateGeminated = (
  cluster: any,
  schema: Schema,
  prevCodaChars: string[],
  ...
  if (!prevCodaChars.some((c) => prevCoda?.includes(c))) {
    return cluster.text;
  }

  const val = schema[schemaKey] as string;
  const modifiedValue = val?.replace(charToRemove, "") ?? "";
  return cluster.text.replace(replaceRegex, `${modifiedValue + val}`);
};

const transliterateFurtivePatach = (syllable: any, schema: Schema, consonantKey: keyof Schema) => {
  const prevText = syllable.prev?.value?.text || "";

  if (syllable.isFinal && prevText) {
    const patah = schema["PATAH"] as string;
    const consonant = schema[consonantKey] as string;
    if (/[יו]/.test(prevText)) {
      const glide = /ו/.test(prevText) ? "w" : "j";
      return glide + patah + consonant;
    }
    return patah + consonant;
  }

  return syllable.text;
};

const getPatachRealization = (
  syllable: any,
  vowelName: string,
  vowelChar: string,
  ...
  if (!allowedVowels.includes(vowelName)) {
    return vowelChar;
  }

  const [onset, , coda] = syllable.structure(true);
  ...
  if (checkEnvironment && nextOnset && /[צץט]/.test(nextOnset)) {
    return "ɑ";
  }
  ...
  ADDITIONAL_FEATURES: [
    ...
    },
    {
      ...
      TRANSLITERATION: (cluster, _, schema) =>
        transliterateGeminated(cluster, schema, ["ת"], "תּ", "TAV_DAGESH", "ʰ"),
    },
    {
      FEATURE: "cluster",
      HEBREW: /פ(?!\u{05b0})/u,
      TRANSLITERATION: (cluster, _, schema) =>
        transliterateGeminated(cluster, schema, ["פ"], "פּ", "PE_DAGESH", "ʰ"),
    },
    {
      FEATURE: "cluster",
      HEBREW: /טּ(?!\u{05b0})/u,
      TRANSLITERATION: (cluster, _, schema) =>
        transliterateGeminated(cluster, schema, ["ט"], "ט", "TET", "ˁ"),
    },
    {
      FEATURE: "cluster",
      HEBREW: /צּ(?!\u{05b0})/u,
      TRANSLITERATION: (cluster, _, schema) =>
        transliterateGeminated(cluster, schema, ["צ"], "צ", "TSADI", "ˁ"),
    },
    {
      FEATURE: "cluster",
      HEBREW: /(כּ|ךּ)(?!\u{05b0})/u,
      TRANSLITERATION: (cluster, _, schema) =>
        transliterateGeminated(cluster, schema, ["כ", "ך"], /כּ|ךּ/u, "KAF_DAGESH", "ʰ"),
    },
    {
      FEATURE: "cluster",
    ...
    },
    {
      ...
      TRANSLITERATION: (syllable, _hebrew, schema) =>
        transliterateFurtivePatach(syllable, schema, "HET"),
    },
    {
      FEATURE: "syllable",
      ...
      PASS_THROUGH: true,
      TRANSLITERATION: (syllable, _hebrew, schema) =>
        transliterateFurtivePatach(syllable, schema, "AYIN"),
    },
    {
      FEATURE: "syllable",
      ...
      PASS_THROUGH: true,
      TRANSLITERATION: (syllable, _hebrew, schema) =>
        transliterateFurtivePatach(syllable, schema, "HE"),
    },
    {
      FEATURE: "syllable",
      ...
      TRANSLITERATION: (syllable, _, schema) => {
        ...
        if (syllable.isAccented && syllable.isClosed) {
          const noLength = (schema["SHUREQ"] as string).replace("ː", "");
    ...
    },
    {
      ...
      TRANSLITERATION: (syllable) => {
        ...
        function determinePatachRealization(vowelChar: string) {
          return getPatachRealization(syllable, vowelName, vowelChar, ["HATAF_PATAH"], true);
        }
    ...
    },
    {
      ...
      TRANSLITERATION: (syllable, _hebrew, schema) => {
        ...
        if (!isGuttural) {
          return transliterateShevaAsVowel(isBackUnrounded() ? "ɑ" : (schema["PATAH"] as string));
        }
        ...
        return transliterateShevaAsVowel(schema[nextVowel as keyof Schema] as string);
      },
    },
    {
      FEATURE: "syllable",
